...
def add(bot, update):
    ...
    # data = get_data_from_pochta(tracking_number)
    # data = parse_data_from_pochta(data)
    # save_to_redis(data)
...
def delete(bot, update):
    print('Пользователь хочет удалить из списка посылок')
    chat_id = 'chat_id:'+ str(update.message.chat_id)
    list_of_parcels = r.get(chat_id)
    list_of_parcels = list_of_parcels.decode('utf-8').split(',')
   

    what_to_del = update.message.text.split()[1]
    if int(what_to_del) > len(list_of_parcels):
        ...
        r.set(chat_id, ','.join(list_of_parcels))

    print('Удалён элемент из списка посылок')
